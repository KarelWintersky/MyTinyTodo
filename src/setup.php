...
if (!$ver) {
    ...
    if (!isset($_POST['installdb']) && !isset($_POST['install'])) {
        $install_db_message = <<<INSTALL_DB_MESSAGE
...
<label><input type=radio name="installdb" value="mysqli" onclick="document.getElementById('mysqlsettings').style.display=''">MySQLi</label><br>
...
INSTALL_DB_MESSAGE;
        exitMessage($install_db_message);
    } elseif (isset($_POST['installdb'])) {
        # Save configuration
        $dbtype = ($_POST['installdb'] == 'mysqli') ? 'mysqli' : 'sqlite';
